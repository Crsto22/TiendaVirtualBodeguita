...
import { ShoppingCart, Trash2, Plus, Minus, Snowflake, Info, ArrowRight, ThermometerSun, X, Recycle, Package } from "lucide-react";
...
export default function CartSidebar({ open, onClose }: CartSidebarProps) {
  ...
  // Función para continuar al checkout
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-between">
                        <div>
                          ...
                          <div className="mt-1.5 flex items-center gap-2">
                            ...
                            {item.mostrar_precio_web !== false && !item.detalle?.startsWith("S/") && (
                        ...
                        <div className="flex items-end justify-between mt-3">
                          {item.tipo_unidad !== 'kilogramo' && (
                            <div className="flex items-center">
                              ...
                              {/* Botón de CAJA (Solo cervezas) */}
                              {['yLBw7BVCpAZa6GNoshBH', 'RJf5z6JxbGR466XreHhv', 'eZny28ndTHvdtrrRJNOA', 'KvvEhbgWfnVGrKJIfZ5q'].includes(item.id) && (
                                <button
                                  onClick={() => {
                                    // Lógica: Saltar al siguiente múltiplo de caja (12)
                                    // Si tengo 5 -> 12. Si tengo 12 -> 24. Si tengo 13 -> 24.
                                    const cajasActuales = Math.floor(item.cantidad / 12);
                                    const nuevasCajas = cajasActuales + 1;
                                    handleUpdateQuantity(item.id, nuevasCajas * 12, item);
                                  }}
                                  className="ml-2 h-9 px-2.5 flex items-center justify-center rounded-xl border border-gray-200 bg-white text-amber-700 hover:bg-amber-50 hover:border-amber-300 shadow-sm relative transition-all active:scale-95 group/btn"
                                  title="Agregar caja (12 unid)"
                                >
                                  <Image
                                    src="/Iconos/cajascerveza.svg"
                                    alt="Caja"
                                    width={18}
                                    height={18}
                                    className="size-4.5 object-contain group-hover/btn:scale-110 transition-transform mr-1"
                                  />
                                  <span className="text-[10px] font-bold">Caja <span className="hidden md:inline">(12 unid)</span></span>

                                  {Math.floor(item.cantidad / 12) > 0 && (
                                    <span className="absolute -top-1.5 -right-1.5 bg-red-500 text-white text-[10px] font-bold size-4 flex items-center justify-center rounded-full shadow-sm border border-white z-10">
                                      {Math.floor(item.cantidad / 12)}
                                    </span>
                                  )}
                                </button>
                              )}
            ...
            <div className="space-y-3 mb-4">
              ...
              <div className="flex justify-between items-end">
                ...
                <span className="text-2xl font-bold text-slate-900 tracking-tight">
                  S/ {calculateCartTotal(items).toFixed(2)}
                </span>
              </div>
